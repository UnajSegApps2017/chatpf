    ...
    public function actionNuevomensaje($idUser, $idContacted) {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->validate()) {
                ...
                $table->messageContent = $gpg->encrypt(new \GPG_Public_Key($usr->publicKeyUser), $model->messageContent);
                $table->headderMessage = $idUser . '- ' . $idContacted;
                //$table->messagedelievered= $model->messagedelievered;
                if ($table->insert()) {
                    return $this->redirect(['index']);
                } else {
                    $model->getErrors();
